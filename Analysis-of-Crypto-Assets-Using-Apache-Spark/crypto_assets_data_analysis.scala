...
val bitcoin_data_path = "file:///home/striker/Desktop/My-Git-Repositories/Exploratory-Data-Analysis/Analysis-of-Cryptocurrency-Using-Apache-Spark/Datasets/bitcoin.csv"
val ethereum_data_path = "file:///home/striker/Desktop/My-Git-Repositories/Exploratory-Data-Analysis/Analysis-of-Cryptocurrency-Using-Apache-Spark/Datasets/ethereum.csv"
val cardano_data_path = "file:///home/striker/Desktop/My-Git-Repositories/Exploratory-Data-Analysis/Analysis-of-Cryptocurrency-Using-Apache-Spark/Datasets/cardano.csv"
...
val bitcoin_final_df = bitcoin_df.withColumn("Date", to_date($"Date", to_date_pattern)).withColumn("Year", year(col("Date"))).withColumn("Month", month(col("Date"))).withColumn("Day_of_Week", dayofweek(col("Date"))).withColumn("pct_change", ((col("Close") - lag("Close", 1).over(Window.partitionBy().orderBy("SNo"))) / col("Close")) * 100.0).drop("SNo")
val ethereum_final_df = ethereum_df.withColumn("Date", to_date($"Date", to_date_pattern)).withColumn("Year", year(col("Date"))).withColumn("Month", month(col("Date"))).withColumn("Day_of_Week", dayofweek(col("Date"))).withColumn("pct_change", ((col("Close") - lag("Close", 1).over(Window.partitionBy().orderBy("SNo"))) / col("Close")) * 100.0).drop("SNo")
val cardano_final_df = cardano_df.withColumn("Date", to_date($"Date", to_date_pattern)).withColumn("Year", year(col("Date"))).withColumn("Month", month(col("Date"))).withColumn("Day_of_Week", dayofweek(col("Date"))).withColumn("pct_change", ((col("Close") - lag("Close", 1).over(Window.partitionBy().orderBy("SNo"))) / col("Close")) * 100.0).drop("SNo")
...
val bitcoin_year_avg_df = bitcoin_final_df.groupBy("Year").agg(mean("Close"), mean("Marketcap")).sort("Year")
bitcoin_year_avg_df.show()

val bitcoin_month_avg_df = bitcoin_final_df.groupBy("Month").agg(mean("Close"), mean("Marketcap")).sort("Month")
bitcoin_month_avg_df.show()

val ethereum_year_avg_df = ethereum_final_df.groupBy("Year").agg(mean("Close"), mean("Marketcap")).sort("Year")
ethereum_year_avg_df.show()

val ethereum_month_avg_df = ethereum_final_df.groupBy("Month").agg(mean("Close"), mean("Marketcap")).sort("Month")
ethereum_month_avg_df.show()

val cardano_year_avg_df = cardano_final_df.groupBy("Year").agg(mean("Close"), mean("Marketcap")).sort("Year")
cardano_year_avg_df.show()

val cardano_month_avg_df = cardano_final_df.groupBy("Month").agg(mean("Close"), mean("Marketcap")).sort("Month")
